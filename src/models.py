...
class LLMOpenAI(LLMModel):
    ...
    def set_response_guideline(self):
        ...
        ** '최고비율'은 '~% 이하' 또는 '~% 미만'과 같은 표현에서 퍼센트 값을 추출합니다. 해당 표현이 없으면 없음으로 반환하세요.  e.g) 자산총액의 80% 이상으로 한다  -> 최고비율 = 없음, 최저비율 = 80
        *** '최저비율'은 '~% 이상' 또는 '~% 초과'와 같은 표현에서 퍼센트 값을 추출합니다. 해당 표현이 없으면 없음으로 반환하세요.  e.g) 자산총액의 30% 이하  -> 최고비율 = 30, 최저비율 = 없음
